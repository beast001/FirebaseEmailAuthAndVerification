                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if(task.isSuccessful()){
                            ...
                            if(isValidDomain(mEmail.getText().toString())){
                                ...
                                FirebaseAuth.getInstance().fetchSignInMethodsForEmail(mEmail.getText().toString().trim()).addOnCompleteListener(
                                        new OnCompleteListener<SignInMethodQueryResult>() {
                                            @Override
                                            public void onComplete(@NonNull Task<SignInMethodQueryResult> task) {
                                                if (task.isSuccessful()){
                                                    Log.d(TAG,"onComplete: RESULTS: " + task.getResult().getSignInMethods().size());
                                                    if (task.getResult().getSignInMethods().size() == 1){
                                                        Log.d(TAG, "onComplete: That email is already in use.");
                                                        hideDialog();
                                                        Toast.makeText(SettingsActivity.this, "Tat email is already in use", Toast.LENGTH_LONG).show();
                                                    }else {
                                                        Log.d(TAG, "onComplete: That email is available.");
                                                        FirebaseAuth.getInstance().getCurrentUser().updateEmail(mEmail.getText().toString().trim())
                                                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                    @Override
                                                                    public void onComplete(@NonNull Task<Void> task) {
                                                                        if (task.isSuccessful()){
                                                                            ...
                                                                            Toast.makeText(SettingsActivity.this, "unable to update email", Toast.LENGTH_LONG).show();
